    ...
    return (
        <Image
            ...
            style={{
                ...
                backgroundColor: 'rgba(0,0,0,0.05)' // Light gray placeholder while loading
            }}
        />
    );
};
...
const WhatsAppInputBar = React.memo(({ colors, isOneTimeMode, onSend, onTypingStatus, onAttachPress }: {
    colors: any;
    isOneTimeMode: boolean;
    ...
    const handleSendClick = useCallback(() => {
        ...
        // Yield to let the input visually clear first
        requestAnimationFrame(() => onSend(msg));
    }, [text, onSend, onTypingStatus]);
    ...
    return (
        <View style={{ flexDirection: 'row', alignItems: 'flex-end', paddingHorizontal: 4, paddingVertical: 8, backgroundColor: 'transparent' }}>
            <View style={{
                flex: 1,
                flexDirection: 'row',
                alignItems: 'center',
                backgroundColor: colors.background, // iOS-like bubble or WhatsApp-like white background inside gray wrapper
                borderRadius: 24,
                paddingHorizontal: 8,
                minHeight: 48,
                marginRight: 8,
                ...
                <TouchableOpacity style={{ padding: 4, marginHorizontal: 2 }} onPress={onAttachPress}>
                    <Icon name="attach-outline" size={26} color="#8E8E93" style={{ transform: [{ rotate: '45deg' }] }} />
                </TouchableOpacity>

                <TextInput
                    style={{ flex: 1, minHeight: 40, maxHeight: 120, paddingVertical: Platform.OS === 'ios' ? 10 : 8, marginHorizontal: 4, color: colors.text, fontSize: 16 }}
                    placeholder={isOneTimeMode ? "One-time view message" : "Message"}
            ...
            <TouchableOpacity
                style={{
                    ...
                    height: 48,
                    borderRadius: 24,
                    backgroundColor: isOneTimeMode ? "#F5A623" : colors.primary, // #10B981 for strictly whatapp green
                    justifyContent: 'center',
                    alignItems: 'center',
                    elevation: 1,
                    shadowColor: '#000',
            ...
            >
                <Icon
                    ...
                    size={22}
                    color="#FFFFFF"
                    style={isTextPresent ? { marginLeft: -2, marginTop: 2 } : {}}
                />
            </TouchableOpacity>
        </View>
    );
...
export default function ChatScreen() {
    ...
    const handleSend = useCallback((textSent: string) => {
        const messageText = String(textSent || '').trim();
        if (!messageText) return;

        const currentReplyId = replyToMessage?.id;
        const wasOneTime = isOneTimeMode;
        ...
        // 2. Immediately scroll to bottom without any timeout hack
        flatListRef.current?.scrollToOffset({ offset: 0, animated: false });

        // 3. Fire the send action immediately. 
        // The store handles optimistic updates instantly and then yields the network request to a setTimeout.
        sendMessage(chatId, messageText, undefined, undefined, wasOneTime, currentReplyId).catch((error: any) => {
    ...
    }, [replyToMessage, isOneTimeMode, isTyping, chatId, sendMessage, messages]);
    ...
    const handleSendFile = async () => {
        ...
        try {
            ...
            if (pickedFile && pickedFile.uri) {
                ...
                // Reset UI states immediately for speed
                setReplyToMessage(null);
                setIsOneTimeMode(false); // Reset OTV mode
                flatListRef.current?.scrollToOffset({ offset: 0, animated: false });

                // Send background network request without awaiting 
                sendMessage(chatId, content, pickedFile.uri, {
                    name: pickedFile.name || 'document',
                    type: pickedFile.type || 'application/octet-stream'
                }, wasOneTime, currentReplyId).catch((error: any) => {
                    console.error('Error sending file:', error);
                    Alert.alert('Upload Failed', error.message || 'Failed to send file');
                });
            }
        } catch (err: any) {
            if (DocumentPicker.isCancel(err)) {
    ...
    // Typing Status isolated for the new optimized sub-component
    const handleTypingStatus = useCallback((isCurrentlyTyping: boolean) => {
    ...
    // Direct O(1) fetch from our pre-inverted Zustand map
    const chatMessagesList = messages.get(chatId) || [];

    // Prevent expensive render calculations on every keystroke
    const renderMessage = useCallback(({ item }: { item: Message }) => {
        ...
        if (item.one_time) {
            ...
            const canView = !isOwn && !isConsumed;
            ...
            return (
                <View style={[styles.messageWrapper, isOwn ? styles.ownMessageWrapper : styles.otherMessageWrapper]}>
                    ...
                    >
                        ...
                        >
                            <View style={[styles.otvIconContainer, { backgroundColor: isOwn ? 'rgba(255,255,255,0.2)' : 'rgba(0,122,255,0.1)' }]}>
                                <Icon
                            ...
                            <View style={{ flex: 1 }}>
                                <Text
                                    style={[
                                        styles.messageText,
                                        { color: isOwn ? '#FFFFFF' : '#1C1C1E', fontWeight: '600' }
                                    ]}
                                >
                    ...
                    <View style={[styles.messageFooter, isOwn ? styles.ownMessageFooter : styles.otherMessageFooter]}>
                    ...
                    >
                        <View
                            style={[
                                ...
                                isOwn ? styles.ownBubble : styles.otherBubble,
                                {
                                    backgroundColor: (isImage && !showText && !item.reply_to && !storyReplyData) ? 'transparent' : (isOwn ? colors.primary : '#FFFFFF'),
                                    borderColor: isOwn ? ((isImage && !showText && !item.reply_to && !storyReplyData) ? 'transparent' : colors.primary) : colors.border,
                                    borderWidth: isOwn ? 0 : 1,
                        ...
                        >
                            {storyReplyData && (
                                <View style={[styles.bubbleReplyContainer, { borderLeftColor: isOwn ? '#FFFFFF' : colors.primary, backgroundColor: isOwn ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)', flexDirection: 'row', alignItems: 'center' }]}>
                                    ...
                                    <View style={{ flex: 1 }}>
                                        <Text style={[styles.bubbleReplySender, { color: isOwn ? '#FFFFFF' : colors.primary }]} numberOfLines={1}>
                                            {item.content === '❤️' ? 'Liked ' : 'Replied to '}{storyReplyData.story_owner || 'story'}
                                        </Text>
                                        <Text style={[styles.bubbleReplyText, { color: isOwn ? 'rgba(255,255,255,0.7)' : colors.textSecondary }]} numberOfLines={1}>
                            ...
                            {item.reply_to && (
                                <View style={[styles.bubbleReplyContainer, { borderLeftColor: isOwn ? '#FFFFFF' : colors.primary, backgroundColor: isOwn ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)' }]}>
                                    <Text style={[styles.bubbleReplySender, { color: isOwn ? '#FFFFFF' : colors.primary }]} numberOfLines={1}>
                            ...
                            {item.media_url && (
                                isVideo ? (
                                    ...
                                    <TouchableOpacity activeOpacity={0.8} onLongPress={(e) => handleMessageLongPress(item, e)}>
                                        <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: isOwn ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.05)', padding: 12, borderRadius: 8, marginBottom: showText ? 8 : 0, maxWidth: 220 }}>
                                            <Icon name="document-text-outline" size={24} color={isOwn ? '#FFFFFF' : colors.primary} style={{ marginRight: 8 }} />
                                            <Text style={{ color: isOwn ? '#FFFFFF' : '#000000', fontSize: 13, flexShrink: 1 }} numberOfLines={2}>
    ...
    }, [user?.id, colors, navigation]); // <-- Close useCallback
                    ...
                    <>
                        ...
                        <View style={[styles.inputContainerWrapper, { backgroundColor: colors.surface, borderTopColor: colors.border, borderTopWidth: isOneTimeMode ? 0 : 1 }]}>
                            ...
                            <WhatsAppInputBar
                                ...
                                onAttachPress={() => setIsAttachMenuVisible(!isAttachMenuVisible)}
                            />
                        </View>
                    </>
                )}
